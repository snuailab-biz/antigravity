...
class FittingRoomAI_GT(object):
    ...
    def volumeProcess(self, pred_depth, pred_mask, pred_info):
        ...
        if not np.isnan(affine_l).any(): # Landmark가 잘못 찍힌 경우 affine transform을 구하지 못하는 경우가 있어 실패
            ...
            left_volume_pad, left_volume = 0, None # Landmark가 잘못 찍힌 경우 affine transform을 구하지 못하는 경우가 있어 실패
        ...
        if not np.isnan(affine_r).any():
            ...
            right_volume_pad, right_volume = 0, None # Landmark가 잘못 찍힌 경우 affine transform을 구하지 못하는 경우가 있어 실패
    ...
    def run(self, image_path):
        ...
        for i, filename in enumerate(filenames):
            ...
            #### VOLUME ####
            volume_pad, volume, left_volume, right_volume = self.volumeProcess(depth_gt, pred_mask, pred_info) # pcd
            ###################
            ...
            valid_lst.append([str(i+1), save_img, save_mask,'', save_point, '', '', volume, left_volume, right_volume, ''])
            ###################
        ...
        df = pd.DataFrame(valid_lst, columns=['ID', 'Image', 'Mask', 'Valid Mask', 'Landmark', 'Valid Landmark', 'Rendering', 'Volume', "Left Volume", "Right Volume", 'Valid Volume'])
        df.to_csv(os.path.join(self.save_path, 'valid.csv'), index=False)


if __name__ == '__main__':
    root_path = r'/home/ljj/data/anti/validation_anti/1/'
    save_path = r'/home/ljj/data/anti/valid_4'

    app = FittingRoomAI_GT(DEBUG=True, save_path=save_path)
