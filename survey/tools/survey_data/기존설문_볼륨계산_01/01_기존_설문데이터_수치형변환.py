...
## 수치형 변환, BMI추가
## 만족도 O 데이터 : 3,716개 존재
## 데이터 저장 : 컵/밴드 사이즈 만족도 7이상 1,274명 데이터
## 모든 사람 데이터 필요시, 저장 전 전처리 후 별도 저장
...
# 데이터 저장
data.to_excel('data/qna_data/qna_기존_수치변환.xlsx', index=False)

# 컵 점수 7이상 인원 수 조회 : 1,592명
# 밴드 점수 7이상 인원 수 조회 : 1,640명
# 컵 점수, 밴드 점수 둘다 7이상 인원 수 조회 : 1,274명
all_7_up = 0
for i in tqdm.tqdm(range(0, len(data))):
    if data['good_bra_score_cup'].iloc[i]>=7 and data['good_bra_score_band'].iloc[i]>=7:
        all_7_up+=1
print('컵/밴드 점수 7 이상',all_7_up)