...
# 사이즈 코리아 데이터 키, 몸무게 정수화
print(sz_kr.head())
sz_kr['키'] = np.round(sz_kr['키'], 0)
sz_kr['몸무게'] = np.round(sz_kr['몸무게'], 0)
print(sz_kr.head())
...
pt5 = sz_kr.pivot_table(['젖꼭지_사이_수평_길이'],index=['젖_가슴_너비'])
pt5.reset_index(inplace=True)
print(pt5)
pt5.info()
df_OUTER_JOIN = pd.merge(df_OUTER_JOIN, pt5, left_on='젖_가슴_너비', right_on='젖_가슴_너비', how='left')
...
pt7 = sz_kr.pivot_table(['젖_가슴_두께'],index=['가슴_두께'])
pt7.reset_index(inplace=True)
print(pt7)
pt7.info()
df_OUTER_JOIN = pd.merge(df_OUTER_JOIN, pt7, left_on='가슴_두께', right_on='가슴_두께', how='left')
df_OUTER_JOIN['젖_가슴_두께'] = df_OUTER_JOIN['젖_가슴_두께'].round(1) # 소수점 첫째자리 반올림

# 8) 젖가슴 둘레 기준 가슴 두께 환산
pt8 = sz_kr.pivot_table(['젖_가슴_아래_두께'],index=['가슴_두께'])
pt8.reset_index(inplace=True)
print(pt8)
pt8.info()
df_OUTER_JOIN = pd.merge(df_OUTER_JOIN, pt8, left_on='가슴_두께', right_on='가슴_두께', how='left')
...
total_data.to_excel('data/qna_data/가공_설문_데이터_0130.xlsx',index=False)
