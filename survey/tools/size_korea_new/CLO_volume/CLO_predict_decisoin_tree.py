...
X = data[['키', '몸무게', 'BMI', '체지방량', '체지방율', '지방조절', '목옆_젖꼭지_길이', '목뒤_젖꼭지_길이',
       ...
       '배꼽수준_허리_둘레', '배꼽수준_허리_너비', '배꼽수준_허리_두께', '배_둘레', '앉은_배_두께',
       '앉은_엉덩이배_두께', '엉덩이_둘레', '엉덩이_두께', '배돌출점기준_엉덩이_둘레', '엉덩이_돌출점_배_돌출점_두께',
       '벽면_몸통_두께', '넙다리_둘레', '유방_볼륨_스누', '유방_볼륨_논문', '가슴_높이_신규',
       '컵_기준치_신규']]
y = data[['유방_볼륨_CLO']]
...
# 특성 중요도
feature_importance = pd.DataFrame(dt_reg.feature_importances_.reshape((1, -1)), columns=X_train.columns, index=['feature_importance'])
print(feature_importance)

## CLO 미존재 데이터 로드, 전처리 후 예측
predict = pd.read_excel('data/size_korea_anti/size_korea_volume_fill_clo.xlsx')
# 볼륨 제외 컬럼 미존재 데이터 삭제
...
predict["CLO_predict"] = pd.DataFrame(data=dt_reg.predict(X), index=predict.index)
predict.to_excel('data/size_korea_anti/clo_predict_dt_reg.xlsx',index=False)
